<?php
	class manutencaoImoveis{
		...
		function listarImoveis(){
			...
			WHERE IMO_NOME like '%".$filtrar_por."%'
			ORDER BY IMO_NOME ASC";
		...
		function listarTarefas(){
			...
			WHERE TAR_TECNICO_CODIGO = ".$_SESSION['sessao_codigo_usuario']." 
			AND TAR_STATUS like '".$status."' ";
			$this->resultado = $this->con->banco->Execute($sql);
		}
		
		function excluirImoveis(){
			$sql = "delete from tbl_imoveis where IMO_CODIGO =".$_REQUEST['codigo'];
		...
		function formularioAlterarImoveis(){
			$sql = "select * from tbl_imoveis where IMO_CODIGO =".$_REQUEST['codigo'];
			$this->resultado = $this->con->banco->Execute($sql);
		...
		function detalhesImoveis(){
			$sql = "select * from tbl_imoveis where IMO_CODIGO =".$_REQUEST['codigo']." AND CLI_CODIGO =".$_REQUEST['clicod'];
			$this->resultado = $this->con->banco->Execute($sql);
			$this->registros = $this->resultado->FetchNextObject();
		}
