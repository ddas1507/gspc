			...
			<div style=" border: 0px solid; ">
				...
				<div style=" border: 0px solid #f00; width: 24px; float: right; ">
					<a href="index.php?tabela=form-list-processos-tarefas&acao=form-list-processos-tarefas&clicod=<? echo $clicod;?>&nome_cliente=<? echo $clinome;?>&img_cliente=<? echo $img_cliente;?>&codigo_imovel=<? echo $imocod;?>&ref_imovel=<? echo $ref;?>&nome_imovel=<? echo $nome;?>" >
						<img width="25px" title="Adicionar Novo Processo" alt="Adicionar Novo Processo" src="http://dionesjobs.com/admin/gspc/images/others/add.png"/>
					</a>
				</div>
			</div>
		</td>
	...
	<tr>
		...
		<table class="tabelas-consulta" >
				<tr style=" font-size: 12px; ">
					...
					<td class="titulo-tabelas" style=" width: 190px; ">Servi&ccedil;o</td>
					<td class="titulo-tabelas">T&eacute;cnico</td>
					<td class="titulo-tabelas">Status</td>
					<td class="titulo-tabelas">A&ccedil;&atilde;o</td>
				...
				<?php
					while($oquefazer->registros = $oquefazer->resultado->FetchNextObject())
					{
						...
						$status2	= $oquefazer->registros->TAR_STATUS2;
						$status	= $oquefazer->registros->TAR_STATUS;
				?>
				<tr class="dadostabelas" onmouseover="this.className='dadostabelasOver'" onmouseout="this.className='dadostabelas'" title="<?php echo $processo;?>">
					<td class="dados-tabelas"><?echo $dataprocessodia.'/'.$dataprocessomes.'/'.$dataprocessoano;?></td>
					<td class="dados-tabelas" ><?echo $servico;?></td>
					...
					<td class="dados-tabelas">
						<?php
							if($status2 == 0 && $processo == 'Protocolo'){?>
								<a href="index.php?tabela=detalhesImoveis-consulta-processos-protocolar&acao=detalhesImoveis-consulta-processos-protocolar&imocod=<? echo $imocod;?>&nprocesso=<? echo $nprocesso;?>&tarcodigo=<? echo $tarcodigo;?>&clicod=<? echo $clicod;?>&servico=<? echo $servico;?>">
									<img src="images/others/ok.png" title="Concluir" border="0" width="24px" />
								</a>
						  <?}
							elseif($status2 == 0){?>
								<a href="index.php?tabela=status-confirmar-tarefas-detalhes-processos&acao=status-confirmar-tarefas-detalhes-processos&imocod=<? echo $imocod;?>&tarcodigo=<? echo $tarcodigo;?>&clicod=<? echo $clicod;?>&servico=<? echo $servico;?>">
									<img src="images/others/ok.png" title="Concluir" border="0" width="24px" />
								</a>
						  <?}
						else
							{?>
								<a href="index.php?tabela=status-desfazer-tarefas-detalhes-processos&acao=status-desfazer-tarefas-detalhes-processos&imocod=<? echo $imocod;?>&tarcodigo=<? echo $tarcodigo;?>&clicod=<? echo $clicod;?>&servico=<? echo $servico;?>">
									<img src="images/others/pendente.png" title="Desfazer" border="0" width="24px" />
								</a>
						  <?}
						?>
					</td>
					<td class="dados-tabelas">
						<a href="index.php?tabela=detalhesImoveis-consulta-servicos-alterar&acao=detalhesImoveis-consulta-servicos-alterar&imocod=<? echo $imocod;?>&tarcodigo=<? echo $tarcodigo;?>&clicod=<? echo $clicod;?>&nprocesso=<? echo $nprocesso;?>">
