<?php
	class manutencaoImoveis{
		...
		function listarTarefasServicos(){
			$sql = "select * FROM tbl_tarefas 
			WHERE TAR_IMOVEL_CODIGO =".$_REQUEST['imocod']." 
			AND TAR_PROCESSO_CODIGO =".$_REQUEST['nprocesso']." ORDER BY DATATAREFAANO,DATATAREFAMES,DATATAREFADIA";
			$this->resultado = $this->con->banco->Execute($sql);
		}
		
		function verificarTarefasServicosConcluidos($imocod, $nprocesso){
		
			$sql = "select COUNT(*) FROM tbl_tarefas 
			WHERE TAR_IMOVEL_CODIGO =".$imocod." 
			AND TAR_PROCESSO_CODIGO =".$nprocesso;
			$this->totalTarefas = $this->con->banco->Execute($sql);
			$controla = explode(')',$this->totalTarefas);
			$this->totalTarefas = $controla[1];
			
			$statusNova = 'Concluido';			
			
			$sql2 = "select COUNT(*) FROM tbl_tarefas 
			WHERE TAR_IMOVEL_CODIGO = $imocod  
			AND TAR_PROCESSO_CODIGO = $nprocesso AND TAR_STATUS = '$statusNova'";
			$this->tarefasConcluidas = $this->con->banco->Execute($sql2);
			$controla = explode(')',$this->tarefasConcluidas);
			$this->tarefasConcluidas = $controla[1];
		}
		...
		function listarTarefasProcessos2(){
			$sql = "select * FROM tbl_tarefasProcessos  
		...
		function gravarAlteracaosImoveisServicos(){
		$dt = date('d/m/Y');
		$hr = gmDate('H:i:s', strtotime("-3 hours"));	
		$data_hora = "$dt - $hr";

			$sql = "update tbl_tarefas set 
			VALORDATAREFA = '".$_POST['valordaTarefa']."',
			DATATAREFADIA = '".$_POST['dataTarefaDia']."',
			DATATAREFAMES = '".$_POST['dataTarefaMes']."',
			DATATAREFAANO = '".$_POST['dataTarefaAno']."',
			PERCENTTAREFA = '".$_POST['percentTarefa']."',
			TAR_DTCAD = '".$data_hora."'

			where TAR_CODIGO = ".$_POST['tarcod'];
			if($this->resultado = $this->con->banco->Execute($sql)){
				//alerta("Imovel alterado com sucesso!");
					return true;
			}
		...
		function formularioAlterarTabelaTarefasProcessos(){
		$dt = date('d/m/Y');
		$hr = gmDate('H:i:s', strtotime("-3 hours"));	
		$data_hora = "$dt - $hr";

			$sql = "update tbl_tarefasProcessos set 
			TAR_PROCESSO_NUMERO = '".$_POST['numeroProcesso']."',
			TAR_DTALT = '".$data_hora."'
		...
		$status2 = 0;
			...
			where TAR_CODIGO = ".$_REQUEST['tarcodigo'];
			if($this->resultado = $this->con->banco->Execute($sql)){
				//alerta("Tarefa Pendenciada com sucesso!");
					return true;
			}
			else{
				alerta("Ocorreu um erro ao tentar concluir a sua tarefa!");
		...
		function formularioAlterarTabelaTarefas(){
			$sql = "select * from tbl_tarefas where TAR_CODIGO =".$_REQUEST['tarcodigo'];
			$this->resultado = $this->con->banco->Execute($sql);
			$this->registros = $this->resultado->FetchNextObject();
		}		
	}
